...
static void destroy_asset(Asset *a) {
    if (a == NULL) return;
...
void destroy_power_assets(PowerAssets *assets) {
    ...
    destroy_asset(assets->grow_pad);
    destroy_asset(assets->grow_balls_size);
    destroy_asset(assets->increase_pad_velocity);
    destroy_asset(assets->increase_ball_velocity);
